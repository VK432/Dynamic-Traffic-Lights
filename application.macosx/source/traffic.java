...
static float constAdvanceRate = 0.01f;
static float constQueueSeperation = 0.025f;

// CONSTANTS - MATH
static float MATH_E = 2.71828182845904523536028747135266249775724709369995f;
...
// Interp function
public int interp(int start, int end, float p){
...
class Route{
  ...
  Route(int turn, int rot){
    ...
    for (int i = 0; i < points.length; i++){
      points[i].p = i * (1.0f / points.length);
    }
  ...
  public void tick(){
    ...
    for (int i = 0; i < carsQueue.size(); i++){
      ...
      if (enabled){
        ...
        c.waitTime++;
        pQueue = points[1].p - (i*constQueueSeperation) - 0.02f;
        if (c.p < pQueue){
          c.p += constAdvanceRate;
        }
      }
    ...
    int expectedCars = PApplet.parseInt(get_cars( rate / constFrameRate )); 
...
class Junction{
  ...
  Junction() {
    ...
    timerEnabled = 0;
    patternIndex = PApplet.parseInt(random(patterns.length));
  ...
  public void drawAverageWaitTimes() {
  ...
  public void drawCurrentPriorityMethod() {
...
// Get number of cars
public int get_cars(float expected){
  float test = random(1);
  float total = 0;
  int k;
...
public void resetJunction() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "traffic" };
    if (passedArgs != null) {
